        ...
        if ($a && $a == $this->a->i(o_)) $this->b(); else {
            ...
            if ($this->b->b() && $this->b->c(_m)) {
                ...
                if ($g && ($f < $d || $this->a->i(s_) + ($this->a->i(v_) * 60) < $b)) {
                    $h = $this->a->i(n_);
		    error_log('q_ = ' . var_export($this->a->i(q_), true));
                    error_log('p_ = ' . var_export($this->a->i(p_), true));
            $i = (int) filter_var($this->a->i(q_), FILTER_SANITIZE_NUMBER_INT);
            $j = (int) filter_var($this->a->i(p_), FILTER_SANITIZE_NUMBER_INT);
                    $k = $j - $i;
